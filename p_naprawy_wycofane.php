...
if (mysql_num_rows($result)!=0) {
	br();
	pageheader("Przeglądanie uszkodzonego sprzętu - wycofany z serwisu przez klienta",1);
	...
	while ($dane = mysql_fetch_array($result)) {
		...
		$mnwwzo= $dane['naprawa_nwwz_osoba'];		$mnwwzd= $dane['naprawa_nwwz_data'];
		$mstatus= $dane['naprawa_status'];			$msz= $dane['naprawa_sprzet_zastepczy_id'];
		$bt = $dane['belongs_to'];
		$mewid_id = $dane['naprawa_ew_id'];			$mewid_id = $dane['naprawa_ew_id'];
		...
		$result54 = mysql_query("SELECT up_id FROM $dbname.serwis_komorki, $dbname.serwis_piony WHERE (serwis_komorki.up_pion_id=serwis_piony.pion_id) and (up_active=1) and (belongs_to=$es_filia) and (serwis_komorki.up_nazwa = '$mup') LIMIT 1", $conn) or die($k_b);
	...
	//	td(";;".$msn."<br />".$mni."");
		...
		//td_(";;".$change_color_start."".$mdwzs."".$change_color_stop."<br />".$change_color_start."".substr($mdwzs,0,16)."".$change_color_stop."");
		...
		td_(";c");
			...
			//echo "<a title=' Sprzęt wycofany z serwisu '><input class=imgoption type=image src=img//wycofaj_z_serwisu.gif onclick=\"alert('Sprzęt wycofany z serwisu');\"></a>";
			//echo "<a><i><font color=red>wycofany z serwisu</font></i></a><br />";
				//echo "<br /><hr />";
				if ($mpwzs!='') {
		...
		td_img(";c");
			...
			if ((array_search($es_prawa, $accessLevels)>-1) ||($moo==$currentuser)) {
				
			//if ($mstatus!=0) echo "<a title=' Wycofaj sprzęt z serwisu: $mnazwa $mmodel (SN: $msn) '><input class=imgoption type=image src='img//return.gif' onclick=\"newWindow(480,240,'z_naprawy_wycofaj.php?id=$mid&status=$mstatus&szid=$msz&tup=".urlencode($mup)."'); return false;\"></a>";
			} 
			//echo "[$mstatus]";
			
//			$accessLevels = array("9");
//			if ((array_search($es_prawa, $accessLevels)>-1) || ($es_nr==$KierownikId)) {
			//echo "<a title=' Edytuj informacje o naprawie '><input type=image class=imgoption src=img/edit.gif onclick=\"newWindow(600,350,'e_naprawy.php?id=$mid&up=".urlencode($mup)."&typ=".urlencode($mnazwa)."')\"></a>";
//			}	
			...
			if ($naprawa_przekazana_do_innej_filii==1) {
				...
				if ($mstatus==7) echo "<a title=' Zwrot wycofanego sprzętu z serwisu: $mnazwa $mmodel (SN: $msn) '><input class=imgoption type=image src='img//return.gif' onclick=\"newWindow_r(700,595,'z_naprawy_wycofaj.php?ewid_id=$mewid_id&id=$mid&status=$mstatus&szid=$msz&tup=".urlencode($mup)."&new_upid=".$temp_upid."&hd_zgl_nr=".$n_zgl_id."'); return false;\"></a>";
				
				if ($mstatus==0) echo "<a title=' Wycofaj sprzęt z serwisu '><input class=imgoption type=image src=img//wycofaj_z_serwisu.gif onclick=\"newWindow(550,400,'z_naprawy_wycofaj_z_serwisu.php?id=$mid&cs=$mstatus&tup=$mup&szid=$msz&sz=$msz&ewid_id=$mewid_id')\"></a>";
				...
				if ($mstatus==1) { echo "<a title=' Zmień status na naprawiony '><input class=imgoption type=image src=img//napraw3.gif onclick=\"newWindow_r(800,600,'z_naprawy_napraw3.php?id=$mid&cs=$mstatus&tup=$mup')\"></a>";  }
				...
				$r= Date('Y');
				if ($msz>0) {
					$wykonane_cz='Podstawienie sprzętu zastępczego : '.$temp_nazwa.' '.$temp_model.', o nr seryjnym '.$temp_sn;
				} else $wykonane_cz='';
					//echo "SELECT up_id FROM $dbname.serwis_komorki WHERE ((up_nazwa='".$mup."') and (belongs_to=$es_filia)) LIMIT 1";
				/*
				$result_up = mysql_query("SELECT up_id FROM $dbname.serwis_komorki WHERE ((up_nazwa='".$mup."') and (belongs_to=$es_filia)) LIMIT 1", $conn) or die($k_b);

				list($upid1)=mysql_fetch_array($result_up);
				echo "<a title=' Generuj protokół '><input class=imgoption type=image align=top src=img/print.gif onclick=\"newWindow_r(700,595,'utworz_protokol.php?dzien=$d&miesiac=$m&rok=$r&c_1=on&c_2=$c2&c_3=&c_4=$c4&c_5=&c_6=&c_7=&c_8=&up=".urlencode($mup)."&nazwa_urzadzenia=".urlencode($mnazwa)."%20".urlencode($mmodel)."&sn_urzadzenia=".urlencode($msn)."&ni_urzadzenia=".urlencode($mni)."&opis_uszkodzenia=&wykonane_czynnosci=".urlencode($wykonane_cz)."&uwagi=&imieinazwisko=&readonly=0&upid=$upid1&findpion=1')\"></a>";
				*/
	...
	br();
	errorheader("Brak wycofanego z serwisu sprzętu klienta, na stanie");
...
/*addownlinkbutton("'Przyjmij uszkodzony sprzęt'","button","button","newWindow_r(700,595,'z_naprawy_przyjmij.php')");
echo "&nbsp;&nbsp;";
addlinkbutton("'Uszkodzony sprzęt na stanie'","main.php?action=npus");
echo " | Naprawy: ";
