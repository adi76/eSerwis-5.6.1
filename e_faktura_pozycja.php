...
if ($submit) {	
	...
	$sql_e1="UPDATE $dbname.serwis_faktura_szcz SET pozycja_nazwa = '$_POST[pnazwa]', pozycja_sn = '$_POST[psn]', pozycja_typ = '$_POST[prodzaj]', pozycja_cena_netto='$cenanetto_cr', pozycja_cena_netto_odsprzedazy='$cenaodsp_cr', pozycja_uwagi='".nl2br($_POST[puwagi])."', pozycja_rodzaj_sprzedazy='$_POST[rodzaj_sprzedazy]', pozycja_gwarancja='$_POST[gwarancja]' WHERE pozycja_id = '$pid' LIMIT 1";
...
while ($newArray = mysql_fetch_array($result)) {
	...
	pageheader("Edycja pozycji na fakturze");
	...
	tbl_empty_row();
		...
		echo "<td><input class=wymagane size=60 maxlength=100 type=text name=pnazwa value='$temp_nazwa'></td>";
		echo "</tr>";
...
?>
<script>
document.getElementById('uco1').value=((Math.round((document.getElementById('pcenanetto').value*document.getElementById('marza').value)*100)/100)); 
SprawdzCeneOdsprzedazy(document.getElementById('pcenanetto').value,document.getElementById('uco1').value,document.getElementById('pcenaodsp').value);
</script>

<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("edu");
  frmvalidator.addValidation("pnazwa","req","Nie wpisałeś nazwy towaru / usługi");
	
  frmvalidator.addValidation("gwarancja","req","Nie wpisałeś ilości miesięcy gwarancji");
  frmvalidator.addValidation("gwarancja","numeric","Użyłeś niedozwolonych znaków w polu \"Gwarancja\"");

	
  frmvalidator.addValidation("prodzaj","dontselect=0","Nie wybrałeś kategorii");
  frmvalidator.addValidation("pcenanetto","req","Nie podałeś ceny netto towaru / usługi");  
  frmvalidator.addValidation("pcenanetto","numericmoney","Użyłeś niedozwolonych znaków w polu Cena netto"); 
  frmvalidator.addValidation("pcenaodsp","req","Nie podałeś ceny netto odsprzedaży");  
  frmvalidator.addValidation("pcenaodsp","numericmoney","Użyłeś niedozwolonych znaków w polu Cena odsprzedaży"); 
