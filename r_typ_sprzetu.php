...
<?php
	...
	pageheader("Typy sprzętu wg słownika",1);
	?><script>ShowWaitingMessage();</script><?php ob_flush(); flush();
	
	$sql="SELECT ewidencja_komputer_opis, COUNT(*) AS ilosc, belongs_to FROM $dbname.serwis_ewidencja ";
	if ($es_m!=1) {
		$sql=$sql."WHERE belongs_to=$es_filia ";
	}
	$sql=$sql."GROUP BY  ewidencja_komputer_opis, belongs_to";
	$result = mysql_query($sql, $conn) or die($k_b);
