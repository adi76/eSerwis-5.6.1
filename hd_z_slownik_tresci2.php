...
//	alert('<?php echo $sl_wybor[0]; ?> | <?php echo $sl_wybor[1]; ?>');
	self.location.href='hd_z_slownik_tresci2.php?akcja=1&p2=<?php echo $sl_wybor[0];?>&p3=<?php echo $sl_wybor[1];?>&p6=<?php echo urlencode($currentuser);?>&refreshed=1';
	</script>
	<?php
	} else {
		$_REQUEST[p2] = $sl_wybor[0];
...
?><script>ShowWaitingMessage();</script><?php ob_flush(); flush();
	...
	if ($showall==0) {
		echo "<a class=paging href=$PHP_SELF?showall=1&paget=$page&hd_rps=$_REQUEST[hd_rps]&p2=$_REQUEST[p2]&p3=$_REQUEST[p3]&p6=".urlencode($_REQUEST[p6])."&akcja=$_REQUEST[akcja]&refreshed=$_REQUEST[refreshed]>Pokaż wszystko na jednej stronie</a>";
	} else {
		echo "<a class=paging href=$PHP_SELF?showall=0&page=$paget&hd_rps=$_REQUEST[hd_rps]&p2=$_REQUEST[p2]&p3=$_REQUEST[p3]&p6=".urlencode($_REQUEST[p6])."&akcja=$_REQUEST[akcja]&refreshed=$_REQUEST[refreshed]>Dziel na strony</a>";	
	}
...
starttable();
if ($_REQUEST[p6]!='') {
	th("40;c;LP|;l;Treść|60;c;Opcje",$es_prawa);
} else {
	th("40;c;LP|;l;Treść|;l;Twórca|60;c;Opcje",$es_prawa);
}
/*if ($_GET[akcja]=='manage') {
...
while ($newArray = mysql_fetch_array($result)) {
	...
	if ($i % 2 != 0 ) echo "<tr id=$i class=nieparzyste onmouseover=rowOver('$i',1); this.style.cursor=arrow onmouseout=rowOver('$i',0) onclick=\"selectRow('$i'); \" onDblClick=\"WybierzTresc2('".nl2br2($temp_tresc)."'); \">";
	if ($i % 2 == 0 ) echo "<tr id=$i class=parzyste onmouseover=rowOver('$i',1); this.style.cursor=arrow onmouseout=rowOver('$i',0) onclick=\"selectRow('$i'); \" onDblClick=\"WybierzTresc2('".nl2br2($temp_tresc)."'); \">";
	...
	if ($_REQUEST[p6]=='') {
		...
		td_(";c");
			if ($_GET[akcja]=='manage') {
				...
				echo "<a title=' Wybierz treść '><input class=imgoption type=image src=img/ok.gif  onclick=\"WybierzTresc2('".nl2br2($temp_tresc)."'); \"></a>";
			}
		_td();
...
include_once('paging_end.php');
	...
	if ($_GET[akcja]!='manage') {
		echo "<input type=button class=buttons value=' Pokaż wszystkie moje' onClick=\" self.location.href='hd_z_slownik_tresci2.php?akcja=1&p2=X&p3=X&refreshed=0&p6=".urlencode($currentuser)."'; \">";
		if (($es_nr==$KierownikId) || ($es_m==1)) {
		echo "<input type=button class=buttons value=' Pokaż wszystkie z filii / oddziału' onClick=\" self.location.href='hd_z_slownik_tresci2.php?akcja=$_GET[akcja]'; \">";
		}
	} else {
		echo "<input type=button class=buttons value=' Pokaż wszystkie moje' onClick=\" self.location.href='hd_z_slownik_tresci2.php?akcja=$_GET[akcja]&p2=X&p3=X&refreshed=0&p6=".urlencode($currentuser)."'; \">";
		if (($es_nr==$KierownikId) || ($es_m==1)) {
			echo "<input type=button class=buttons value=' Pokaż wszystkie z filii / oddziału' onClick=\" self.location.href='hd_z_slownik_tresci2.php?akcja=$_GET[akcja]&p2=X&p3=X'; \">";
		}
	}
	echo "</span>";
