...
$sql1 = "SELECT * FROM $dbname.serwis_faktura_szcz WHERE (pozycja_nr_faktury='$id') and (pozycja_status>-2) ORDER BY pozycja_id ASC";
$result1 = mysql_query($sql1, $conn) or die($k_b);
...
while ($newArray = mysql_fetch_array($result1)) {
	...
	if ($_REQUEST[pozid]!='') {
		...
		if ($temp_rs=='Usługa') {
			list($kolorgrupy)=mysql_fetch_array(mysql_query("SELECT rola_kolor FROM $dbname.serwis_slownik_rola WHERE rola_nazwa='$temp_rs' LIMIT 1",$conn));
...
if (($temp_statusf==1) || ($temp_statusf==9)) { 
	...
	*/
	...
	if ($temp_rs!='') {
		...
		if ($temp_rs=='Usługa') {
			$cnt_uslugi++;
			$sum_uslugi=$sum_uslugi+$temp_cenanetto;
		} else {
			$cnt_towary++;
			$sum_towary=$sum_towary+$temp_cenanetto;
		}
		
	} else { 
		echo "<br /><font color=grey><a class=normalfont title=' Nie określono rodzaju sprzedaży ' href=#>-</a></font>";
		$cnt_nieokreslone++;
		$sum_nieokreslone=$sum_nieokreslone+$temp_cenanetto;
	}	
...
echo "Łączna kwota <b>towarów</b> (sztuk <b>".$cnt_towary."</b>) na fakturze: <b>".correct_currency($sum_towary)." zł</b><br />";
echo "<font color='$kolorgrupy'>Łączna kwota <b>usług</b> (sztuk <b>".$cnt_uslugi."</b>) na fakturze: <b>".correct_currency($sum_uslugi)." zł</b><br /></font>";
if ($cnt_nieokreslone>0) echo "Łączna kwota pozycji nieokreślonych (sztuk <b>".$cnt_nieokreslone."</b>) na fakturze: <b>".correct_currency($sum_nieokreslone)." zł</b><br />";

$sum_towary_i_uslugi = $sum_towary + $sum_uslugi + $sum_nieokreslone;
echo "<font color='grey'>Łączna kwota z podsumowania wg typu: <b>".correct_currency($sum_towary_i_uslugi)." zł</b></font><br />";
echo "<hr />";
endbuttonsarea();
...
if ($temp_knf!='') {
	...
	if ($roznica>0) echo "<font color=$set_color>Wartość wpisanych pozycji na fakturze jest mniejsza niż wpisanej kwoty faktury netto o: <b>".correct_currency($roznica)." zł</b>&nbsp;</font><br /><br />";
	if ($roznica==0) echo "<b><font color=green>Wpisana kwota netto faktury jest zgodna z wartością wpisanych pozycji</b></font>&nbsp;<br /><br />";
	if ($roznica<0) echo "<font color=$set_color>Wartość wpisanych pozycji przekracza wpisaną kwotę faktury netto o: <b>".correct_currency(abs($roznica))." zł</b>&nbsp;</font><br /><br />";
}
endbuttonsarea();
