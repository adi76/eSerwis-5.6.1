...
if ($submit) {
	...
	$sql = "UPDATE $dbname_hd.hd_zgloszenie_szcz SET zgl_szcz_oferta_wyslana=1 WHERE (zgl_szcz_id='$_REQUEST[hd_oferta_zgl_szcz_id]') LIMIT 1";
	$result = mysql_query($sql, $conn_hd) or die($k_b);
	
	$sql = "INSERT INTO $dbname_hd.hd_zgloszenie_oferty VALUES ('',$_REQUEST[hd_oferta_zgl_szcz_id],'$_REQUEST[hd_oferta_data]','$_REQUEST[hd_oferta_numer]','$_REQUEST[hd_oferta_uwagi]',1,'$currentuser',$es_filia)";
	...
	if (mysql_query($sql, $conn_hd)) {
		...
		if ($jest_powiazana_naprawa>0) {
			$sql1 = "UPDATE $dbname_hd.hd_naprawy_powiazane SET hdnp_oferta_wyslana=1 WHERE (hdnp_id='$jest_powiazana_naprawa') LIMIT 1";
			$result1 = mysql_query($sql1, $conn_hd) or die($k_b);
		}
			
		?><script>if (opener) opener.location.reload(true); self.close(); document.getElementById("OfertaWyslanaButton").style.display='none'; </script><?php
...
pageheader("Potwierdzenie wysłania oferty do klienta");
starttable();
...
tbl_empty_row(2);
	...
	tr_();
		...
		td_(";;;");
			echo "<textarea name=hd_oferta_uwagi cols=35 rows=5 /></textarea>";
		_td();
	_tr();		
	tbl_empty_row(2);
endtable();
