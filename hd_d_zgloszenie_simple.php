...
switch ($_body) {
	case "N" : echo "<body OnLoad=\"document.getElementById('hdnzhadim').focus(); MakePodkategoriaList_simple('3'); GenerateSubPodkategoriaList(document.getElementById('kat_id').value,document.getElementById('podkat_id').value); GenerateOnClickEventForSlownikTresci(); \" onUnload=\"eraseCookie(self.name); eraseCookie('current_window_name'); \" />"; break;
...
include_once('systemdate.php');
	...
	tr_();
		...
		td_(";;;");		
			...
			echo " maxlength=".$HADIM_max." size=".$HADIM_width." onKeyPress=\"return filterInput(1, event, false); \" onFocus=\"this.select();\" onBlur=\"if (this.value>0) { document.getElementById('getHDIM').style.display='';} else { document.getElementById('getHDIM').style.display='none'; }\" />";
			
			if ($es_hdim==1) {
				echo "&nbsp;<input type=button class=buttons value='Otwórz zgłoszenie HDIM w nowym oknie' onClick=\"getHDIM_opis(document.getElementById('hdnzhadim').value); return false;\" id=getHDIM style='display:none;'>";
			//	echo "&nbsp;<input type=button class=buttons value='Pobierz dane z HDIM' id=getHDIM style='display:none;'>";
			}
...
echo "<div id=div_test></div>";
	...
	tr_();
		...
		td_(";;;");
				...
				echo "<input class=wymagane type=text size=70 maxlength=50 name=up_list id=up_list onFocus=\"$(this).select(); \" onBlur=\"SprawdzKomorke(this.value); this.value=this.value.toUpperCase(); return false; \" />";
...
//		$zgloszenie_kat='3';
	...
	echo "<input type=hidden name=priorytet_id value='$zgloszenie_priorytet'>";
		...
		if (($_REQUEST[fromtask]==1) && ($_REQUEST[zadanie]!='')) {
			...
			tr_();
				td("140;rt;Podkategoria");
				td_(";;;");	
					echo "<select class=wymagane id=podkat_id name=podkat_id onChange=\"GenerateSubPodkategoriaList(document.getElementById('kat_id').value,document.getElementById('podkat_id').value); StatusChanged(document.getElementById('kat_id').value,document.getElementById('status_id').value,'Status'); GenerateOnClickEventForSlownikTresci(); BudujListeDatRozpoczecia(document.getElementById('kat_id').value); MakeListaDat(0); ShowHints(document.getElementById('kat_id').value,this.value); \" onBlur=\"\"/>\n";
					echo "<option value=''></option>\n";		
					echo "</select>\n";			
				_td();
			_tr();
	...
	echo "</tr>";
		...
		echo "<tr id=WieleOsob style='display:none;'>";
			td(";;");
			td_(";;;");
			//	echo "<span id=PowiazaneZWyjazdem style=display:''>";
				echo "<input class=border0 style='display:none;' type=checkbox name=WieleOsobCheck id=WieleOsobCheck onClick=\"if (this.checked) { document.getElementById('WieleOsobWybor').style.display=''; } else { document.getElementById('WieleOsobWybor').style.display='none'; } \" />";
		...
		echo "</tr>";
		//tbl_empty_row(1);
		echo "<tr id=WieleOsobWybor style='display:none;'>";
...
<?php
// echo file_get_contents("https://sd.cit.net.pp/helpdesk/issue.php?id=2265888");
