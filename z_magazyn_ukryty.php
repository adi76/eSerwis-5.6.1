...
<?php
if ($es_m==1) {
	$result = mysql_query("SELECT magazyn_id,magazyn_nazwa,magazyn_model,magazyn_sn,magazyn_ni,magazyn_uwagi,magazyn_uwagi_sa,magazyn_status, belongs_to FROM $dbname.serwis_magazyn WHERE magazyn_status='3' and belongs_to=$es_filia", $conn) or die($k_b);
} else {
	if ($wybierz_typ=='') {
		$result = mysql_query("SELECT magazyn_id,magazyn_nazwa,magazyn_model,magazyn_sn,magazyn_ni,magazyn_uwagi,magazyn_uwagi_sa,magazyn_status,belongs_to FROM $dbname.serwis_magazyn WHERE magazyn_status='3' and belongs_to=$es_filia ORDER BY magazyn_nazwa ASC", $conn) or die($k_b);
	} else {
		$result = mysql_query("SELECT magazyn_id,magazyn_nazwa,magazyn_model,magazyn_sn,magazyn_ni,magazyn_uwagi,magazyn_uwagi_sa,magazyn_status,belongs_to FROM $dbname.serwis_magazyn WHERE magazyn_status='3' and belongs_to=$es_filia and (magazyn_nazwa='$wybierz_typ') ORDER BY magazyn_nazwa ASC", $conn) or die($k_b);
	}
}

if (mysql_num_rows($result)!=0) {
	br();
	pageheader("Przeglądanie ukrytego sprzętu serwisowego",1);
	...
	//echo "&nbsp;&nbsp;";
	if ($kolor!=1) {
		echo "<input id=with_color type=button class=buttons value='Koloruj wg typu' onClick=\"self.location.href='main.php?action=pus&wybierz_typ=$wybierz_typ&kolor=1'\" />";
	} else {
		echo "<input id=without_color type=button class=buttons value='Bez kolorowania wg typu' onClick=\"self.location.href='main.php?action=pus&wybierz_typ=$wybierz_typ&kolor=0'\" />";
	}
	echo "</span>";
	...
	echo "<select name=wybierz_typ onChange='document.location.href=document.magazyn.wybierz_typ.options[document.magazyn.wybierz_typ.selectedIndex].value'>";
		...
		echo "value='main.php?action=pus&wybierz_typ=&kolor=$kolor'>Cały sprzęt</option>\n";	
		while (list($temp_magazyn_nazwa)=mysql_fetch_array($wynik_lista_typ)) {
			echo "<option "; 
			if ($wybierz_typ==$temp_magazyn_nazwa) echo "SELECTED ";
			echo "value='main.php?action=pus&wybierz_typ=".urlencode($temp_magazyn_nazwa)."&kolor=$kolor'>$temp_magazyn_nazwa</option>\n";	
		}
	echo "</select>";
	echo "<input type=hidden name=action value=pus>";
	...
	while (list($mid, $mnazwa, $mmodel, $msn, $mni, $muwagi, $muwagisa,$mstatus,$bt) = mysql_fetch_array($result)) {
		...
		if ($_GET[kolor]!=1) {
			tbl_tr_highlight_dblClick_z_magazyn_ukryty($i);
		} else {
			...
			tbl_tr_highlight_dblClick_z_magazyn_ukryty_kolor($i,$kolorgrupy);
		}
		...
		echo "<input type=hidden id=pozname$i value='$mnazwa'>";
			...
			_td();
			td_img(";c");
				echo "<a title=' Odkryj sprzęt $mnazwa $mmodel o numerze seryjnym $msn '><input class=imgoption type=image src=img/odkryj.png onclick=\"newWindow($dialog_window_x,$dialog_window_y,'z_magazyn_odkryj.php?select_id=$mid')\"></a>";
			_td();
		_tr();
	}
endtable();
	...
	{
		br();
		errorheader("Żaden sprzęt serwisowy nie jest ukryty");
		?>
