...
if (mysql_num_rows($result)!=0) {
	br();
	pageheader("Przeglądanie sprzętu, który może być zarezerwowany");
	?><script>ShowWaitingMessage('','a');</script><?php ob_flush(); flush();	
	startbuttonsarea("center");
	echo "<span style='float:left'>";
	//echo "&nbsp;&nbsp;";
	if ($kolor!=1) {
		echo "<input id=with_color type=button class=buttons value='Koloruj wg typu' onClick=\"self.location.href='main.php?action=rs&wybierz_typ=$wybierz_typ&kolor=1'\" />";
	} else {
		echo "<input id=without_color type=button class=buttons value='Bez kolorowania wg typu' onClick=\"self.location.href='main.php?action=rs&wybierz_typ=$wybierz_typ&kolor=0'\" />";
	}
	echo "</span>";
	...
	echo "<select name=wybierz_typ  onChange='document.location.href=document.magazyn.wybierz_typ.options[document.magazyn.wybierz_typ.selectedIndex].value'>";
		...
		echo "value='main.php?action=rs&wybierz_typ=&kolor=$kolor'>Cały sprzęt</option>\n";	
		while (list($temp_magazyn_nazwa)=mysql_fetch_array($wynik_lista_typ)) {
			echo "<option "; 
			if ($wybierz_typ==$temp_magazyn_nazwa) echo "SELECTED ";
			echo "value='main.php?action=rs&wybierz_typ=".urlencode($temp_magazyn_nazwa)."&kolor=$kolor'>$temp_magazyn_nazwa</option>\n";	
		}
	echo "</select>";
	echo "<input type=hidden name=action value=rs>";
	...
	while (list($mid, $mnazwa, $mmodel, $msn, $mni, $muwagi, $muwagisa,$mstatus) = mysql_fetch_array($result)) {
		if ($_GET[kolor]!=1) {
			tbl_tr_highlight_dblClick_p_magazyn_do_rezerwacji($i);
		} else {
		...
		$i+=1;
			...
			_td();
			td_img(";c");
				echo "<a title=' Zarezerwuj sprzęt $mnazwa $mmodel o numerze seryjnym $msn '><input class=imgoption type=image src=img/zarezerwuj.gif  onclick=\"newWindow($dialog_window_x,$dialog_window_y,'z_magazyn_zarezerwuj.php?select_id=$mid')\"></a>";
			_td();
		_tr();
    }
endtable();
...
//addlinkbutton("'Zarezerwuj sprzęt'","main.php?action=rs");
