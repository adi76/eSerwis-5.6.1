...
include_once('cfg_helpdesk.php');
	...
	while ($dane88 = mysql_fetch_array($result88)) {
		...
		if ($today=='') {
			$span_notatki_moje.= "' href=# onclick=\"newWindow_r(600,400,'hd_e_note.php?noteid=$temp_id88&edit=$note_edit&creator=$temp_creator88&cr_date=".unescape($temp_creation_date)."&cr_name=".$creator_name."&refresh=1'); $('#count_notatki_moje').load('hd_count_notes.php?user_id=$es_nr&randval='+ Math.random()); $('#notatki').load('hd_refresh_notes_na_startowej.php?userid=$es_nr&randval='+ Math.random()); return false; \">";
		} else {
			$span_notatki_moje.= "' href=# onclick=\"newWindow_r(600,400,'hd_e_note.php?noteid=$temp_id88&edit=$note_edit&creator=$temp_creator88&cr_date=".unescape($temp_creation_date)."&cr_name=".$creator_name."&today=$today&refresh=1'); $('#count_notatki_moje').load('hd_count_notes.php?user_id=$es_nr&randval='+ Math.random()); $('#notatki').load('hd_refresh_notes_na_startowej.php?userid=$es_nr&randval='+ Math.random()+'&today=$today&nastartowej=1'); return false; \">";
