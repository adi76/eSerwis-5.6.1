...
if ($count!=0) {
	...
	pageheader("Sprawny sprzęt dostępny w wybranej lokalizacji<br /><br />Wybrana lokalizacja : <b>$temp_pion $upnazwa</b><br />Wybrany typ sprzętu : <b>$rolanazwa</b><br />");	
				...
				//  if ($jakistatus==5) echo "sprzęt wrócił z naprawy";
					...
					if ($jakistatus==8) {
						...
						echo "><img class=imgoption src=img/wycofaj_z_serwisu.gif border=0 width=16 width=16></a>";
							...
							$_COOKIE['tuwagi_'.$_REQUEST[ewid_id].'']='';
//							echo "$_REQUEST[from] | $_REQUEST[hd_nr]";
						}
			
						//echo "<input class=imgoption type=image src=img//napraw.gif title=' Przyjmij wybrany sprzęt do naprawy ' onclick='window.location=\"z_naprawy_uszkodzony.php?id=$eeid&ts=d&auto=1&pionnazwa=$temp_pion&tup=".urlencode($upnazwa)."&up=".urlencode($nazwaup)."&tnazwa=".urlencode($rolanazwa)."&tmodel=".urlencode($edo)."&tsn=".urlencode($edsn)."&tni=".urlencode($edni)."\"'>";			
				...
				} else if ($rolanazwa=='Monitor')
					{
						...
						//  if ($jakistatus==5) echo "sprzęt wrócił z naprawy";
							...
							echo "<input class=imgoption type=image src=img//napraw.gif title=' Przyjmij wybrany sprzęt do naprawy ' onclick='window.location=\"z_naprawy_uszkodzony.php?ewid_id=$eeid&id=$eeid&ts=m&auto=1&pionnazwa=$temp_pion&tup=".urlencode($upnazwa)."&up=".urlencode($upnazwa)."&tnazwa=".urlencode($rolanazwa)."&tmodel=".urlencode($emo)."&tsn=".urlencode($emsn)."&tni=".urlencode($ekni)."&tresc_zgl=".urlencode($_REQUEST[tresc_zgl])."&new_upid=".urlencode($_REQUEST[new_upid])."&from=$_REQUEST[from]&hd_nr=$_REQUEST[hd_nr]\"'>";
