...
<?php
if ($es_m==1) {
	$result = mysql_query("SELECT * FROM $dbname.serwis_naprawa WHERE (naprawa_status='2')", $conn) or die($k_b);
} else {
	$result = mysql_query("SELECT * FROM $dbname.serwis_naprawa WHERE (naprawa_status='2') and ((belongs_to=$es_filia) or ((naprawa_przekazanie_naprawy_do=$es_filia) and (naprawa_przekazanie_zakonczone=0)))", $conn) or die($k_b);
}
if (mysql_num_rows($result)!=0) {
	br();
	pageheader("Przeglądanie uszkodzonego sprzętu - naprawy w serwisach zewnętrznych na rynku lokalnym",1);
	...
	starttable();
	if ($es_m==1) {
		th("30;c;LP|;;Nazwa Model<br />Numer seryjny, inwentarzowy|;;Pobrano z|;;Przyjęte przez<br />Data przyjęcia|;;Wysłane przez<br />Data wysyłki|;c;Nazwa serwisu<br />Sprzęt zastępczy<br/>Termin upływa za|;c;Uwagi<br /><sub>Filia</sub>|;c;Opcje",$es_prawa);
	} else {
	...
	while ($dane = mysql_fetch_array($result)) {
		...
		//td_("50%;nw;<b>".$mnazwa." ".$mmodel."</b><br /><sub>".$msn.", ".$mni."</sub>");
		echo "<td>";
		...
		td_(";;");
				...
				echo "<a class=normalfont title=' Szczegółowe informacje o $temp_pion_nazwa $mup ' onclick=\"newWindow_r(700,600,'p_komorka_szczegoly.php?id=$temp_up_id')\" href=#><b>".$change_color_start."$temp_pion_nazwa $mup".$change_color_stop."</b></a><br />";				
				//echo "".$msn."<br />".$mni."";
			_td();
			...
			td_(";c");
				...
				if ($mstatus=='1') { 
					...
					echo "<a class=normalfont title=' Telefon do firmy : $fztel ' onclick=\"newWindow(600,280,'p_fz_szczegoly.php?id=$fs_id')\" href=#><b>$fs</b></a>";
					if ($mnfk!='-1') {
				...
				if ($msz>0) {
					...
					//echo "<b>$temp_nazwa $temp_model</b><br />SN: $temp_sn</sub>";
					...
					if ($naprawa_przekazana_do_innej_filii==1) {
						// informacje o tym że sprzęt jest w innej filii / oddziale
							if ($n_przekaz_do==$es_filia) {
								...
								echo "<a title=' Zmień serwis '><input class=imgoption type=image src=img//zmienserwis.gif onclick=\"newWindow(590,270,'z_naprawy_zmien_serwis.php?nid=$mid&cs=$mstatus&tup=".urlencode($mup)."&staryserwis=$fs_id&fk=".urlencode($mnfk)."&fs=".urlencode($mnfs)."&fknlp=$mnnlp')\"></a>";
						...
						echo "<a title=' Przekaż sprzęt do naprawy do innej filli/oddziału '><input class=imgoption type=image src=img//naprawa_przekaz.gif onclick=\"newWindow(600,350,'z_naprawa_przekaz.php?id=$mid&cs=$mstatus&tup=".urlencode($mup)."&cs=2&hdzgl=$n_zgl_id')\"></a>"; 
