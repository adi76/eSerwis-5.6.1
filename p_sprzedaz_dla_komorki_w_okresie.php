...
if ($submit) { 
	...
	// ustalenie pełnej nazwy komórki po id komórki
	// $sql111 = "SELECT pion_nazwa,up_nazwa FROM $dbname.serwis_piony, $dbname.serwis_komorki WHERE (serwis_piony.pion_id=serwis_komorki.up_pion_id) and (serwis_komorki.up_id=$_REQUEST[sel_up]) LIMIT 1";
	// list($_p,$_k)=mysql_fetch_array(mysql_query($sql111,$conn));
	
	$pelna_nazwa_up = toUpper($_REQUEST[sel_up]);
	...
	*/
	...
	if (($okres_od!="") && ($okres_do!="")) {
		if (!$bez_hd) {
			...
			$sql_a .= " AND (UPPER(CONCAT(serwis_sprzedaz.sprzedaz_pion_nazwa,' ',serwis_sprzedaz.sprzedaz_up_nazwa) LIKE '%$pelna_nazwa_up%')) ";
			...
			$sql_a .= " AND (UPPER(CONCAT(serwis_sprzedaz.sprzedaz_pion_nazwa,' ',serwis_sprzedaz.sprzedaz_up_nazwa) LIKE '%$pelna_nazwa_up%')) ";
			if (($_REQUEST[tuser]!='all') && ($_REQUEST[tuser]!='')) $sql_a .= " and (serwis_sprzedaz.sprzedaz_osoba='$_REQUEST[tuser]') ";
			$sql_a .= " ORDER BY serwis_sprzedaz.sprzedaz_id DESC, sprzedaz_unique ASC"; 			
		}
	}
...
//	echo $sql_a;
	...
	if ($count_rows>0) {
		pageheader("Raport ze sprzedaży towarów / usług w podanym okresie dla wybranej komórki",1,1);

		?><script>ShowWaitingMessage();</script><?php ob_flush(); flush();
		
		if ($tuser!="all") infoheader("dla komórki:<br /><b>".$pelna_nazwa_up."</b><br />w okresie<br /><b>".$okres_od." - ".$okres_do."</b>");
...
*/	
	...
	if (($_REQUEST[okres_od]>'2011-09-20') && ($_REQUEST[okres_do]>'2011-09-21')) {
		...
		if ($count_rows5>0) {
			...
			echo " value='$PHP_SELF?okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&tuser=$_REQUEST[tuser]&submit=$_REQUEST[submit]&dlazgl=&sel_up=".urlencode($_REQUEST[sel_up])."'>wg zgłoszenia</option>\n";
			while ($newArray44 = mysql_fetch_array($result5)) {
				$temp_iin = $newArray44['wp_zgl_id'];		
				if ($temp_iin!='0') {
					echo "<option";
					if ($temp_iin==$_REQUEST[dlazgl]) echo " SELECTED ";
					echo " value='$PHP_SELF?okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&tuser=$_REQUEST[tuser]&submit=$_REQUEST[submit]&dlazgl=".urlencode($temp_iin)."&sel_up=".urlencode($_REQUEST[sel_up])."'>$temp_iin</option>\n"; 
				}
			}
			echo "</select>";
		}
		...
		echo "&nbsp;<select ";
			...
			echo "<option value='$PHP_SELF?okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&tuser=all&submit=$_REQUEST[submit]&dlazgl=$_REQUEST[dlazgl]&sel_up=".urlencode($_REQUEST[sel_up])."' "; 
			...
			while (list($temp_imie,$temp_nazwisko) = mysql_fetch_array($result6)) {
				...
				echo "<option value='$PHP_SELF?okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&tuser=".urlencode($iin)."&submit=$_REQUEST[submit]&dlazgl=$_REQUEST[dlazgl]&sel_up=".urlencode($_REQUEST[sel_up])."' ";
				if ($_REQUEST[tuser]==$iin) echo " SELECTED ";
				echo ">$temp_imie $temp_nazwisko</option>\n"; 
			}
			echo "</select>\n";
		
			echo "<input type=button class=buttons value='Domyślny widok' style='padding:2px; margin:2px;' onClick=\"document.location.href='$PHP_SELF?okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&sel_up=".urlencode($_REQUEST[sel_up])."&submit=$_REQUEST[submit]&tuser=all'; \" />";
		endbuttonsarea();
		echo "</form>";
...
addlinkbutton("'Zmień kryteria'","main.php?action=zsdwk&okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&sel_up=".urlencode($_REQUEST[sel_up])."");
echo "</span>";
...
br();
pageheader("Pokaż sprzedaż w okresie dla wybranej komórki");
starttable("350px");
echo "<form name=ruch action=p_sprzedaz_dla_komorki_w_okresie.php method=GET onsubmit=\"if (document.getElementById('sel_up').value=='') { alert('Nie podałeś komórki'); document.getElementById('sel_up').focus(); return  false; } return validateForm(); \" >";
tbl_empty_row();
	tr_();
		td_colspan(2,'c');
	...
	tr_();
		td_colspan(2,'c');
			echo "<b>Podaj komórkę<br /><br /></b>";
		_td();
	_tr();
	tr_();
		td_colspan(2,'c');
		
			echo "<input class=wymagane type=text size=70 maxlength=50 name=sel_up id=sel_up onBlur=\"this.forms.submit();\" value='$_REQUEST[sel_up]'>";
			/*
			...
			echo "<select class=wymagane name=sel_up onChange=\"if (this.value=='') { document.getElementById('submit').style.display='none'; } else { document.getElementById('submit').style.display=''; } \">\n"; 		
...
endtable();
startbuttonsarea("center");
echo "<input class=buttons type=submit id=submit name=submit value='Pokaż'>";
endbuttonsarea();
_form();

?>
<script language="JavaScript">
document.getElementById('sel_up').focus();
