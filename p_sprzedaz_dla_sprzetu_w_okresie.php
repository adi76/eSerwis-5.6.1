...
if ($submit) { 
	...
	$_REQUEST[sprzet] = toUpper($_REQUEST[sprzet]);
	...
	if (($okres_od!="") && ($okres_do!="")) {
		if (!$bez_hd) {
			$sql_a = "SELECT wp_sprzedaz_fakt_szcz_id, wp_sprzet_opis, wp_sprzet_sn, wp_sprzet_ni FROM $dbname_hd.hd_zgl_wymiany_podzespolow WHERE (hd_zgl_wymiany_podzespolow.belongs_to=$es_filia) and (hd_zgl_wymiany_podzespolow.wp_id<>0) and ";
			
			// ((SUBSTRING(hd_zgl_wymiany_podzespolow.wp_sprzet_data,1,10)) BETWEEN '$okres_od' AND '$okres_do') ";
			//$sql_a .= " AND (UPPER(CONCAT(serwis_sprzedaz.sprzedaz_pion_nazwa,' ',serwis_sprzedaz.sprzedaz_up_nazwa)='$pelna_nazwa_up')) ";
			//if (($_REQUEST[tuser]!='all') && ($_REQUEST[tuser]!='')) $sql_a .= " and (serwis_sprzedaz.sprzedaz_osoba='$_REQUEST[tuser]') ";
			$sql_a .= " ((UPPER(wp_sprzet_ni) LIKE '%$_REQUEST[sprzet]%') or (UPPER(wp_sprzet_sn) LIKE '%$_REQUEST[sprzet]%'))"; 
			$sql_a .= " ORDER BY hd_zgl_wymiany_podzespolow.wp_sprzet_data DESC"; 
		}
	}
...
if ($bez_hd==false) {
	...
	list($pfid, $_wp_o, $_wp_sn, $_wp_ni)=mysql_fetch_array($result_a);	
	
	$count_rows = mysql_num_rows($result_a);

	$wyswietl_liste=false;
	if ($count_rows>0) {
		//list($_p_status, $_p_data_sprz, $_p_gwarancja)=mysql_fetch_array(mysql_query("SELECT pozycja_status, pozycja_datasprzedazy, pozycja_gwarancja FROM $dbname.serwis_faktura_szcz WHERE (pozycja_id='$temp_poz_pid')",$conn));	
			
		list($temp_data, $temp_osoba, $temp_umowa, $temp_pion, $temp_up, $temp_rodzaj, $temp_status, $temp_uwagi, $temp_fakt_id)=mysql_fetch_array(mysql_query("SELECT sprzedaz_data, sprzedaz_osoba, sprzedaz_umowa_nazwa, sprzedaz_pion_nazwa, sprzedaz_up_nazwa, sprzedaz_rodzaj, sprzedaz_status, sprzedaz_uwagi, sprzedaz_faktura_id FROM $dbname.serwis_sprzedaz WHERE (sprzedaz_pozycja_id='$pfid')",$conn));	

	   if ((($temp_status=='1') || ($temp_status=='9')) && ($temp_data>=$okres_od) && ($temp_data<=$okres_do)) $wyswietl_liste=true;
	}
		
	if ($wyswietl_liste) {
		pageheader("Raport ze sprzedaży towarów / usług w okresie dla sprzętu",1,1);

		?><script>ShowWaitingMessage();</script><?php ob_flush(); flush();
		//$_wp_o, $_wp_sn, $_wp_ni
		infoheader("dla sprzętu:<br /><b>$_wp_o</b><br />SN: <b>$_wp_sn</b>, NI: <b>$_wp_ni</b><br /><br />w okresie<br /><b>".$okres_od." - ".$okres_do."</b>");
...
*/
		...
		while ($newArray = mysql_fetch_array($result_a)) {		
			...
			//$temp_poz_nazwa		= $newArray['wp_sprzet_opis'];			
			$temp_poz_sn		= $newArray['wp_sprzet_sn'];			
			$temp_poz_ni		= $newArray['wp_sprzet_ni'];			
			
			$temp_poz_pid		= $newArray['wp_sprzedaz_fakt_szcz_id'];
			$temp_typ_podzespolu = $newArray['wp_typ_podzespolu'];
	
			$temp_id = $temp_poz_pid;
			
			list($_p_status, $_p_data_sprz, $_p_gwarancja, $_p_sn)=mysql_fetch_array(mysql_query("SELECT pozycja_status, pozycja_datasprzedazy, pozycja_gwarancja, pozycja_sn FROM $dbname.serwis_faktura_szcz WHERE (pozycja_id='$temp_poz_pid')",$conn));	
			
			//if ($_p_sn!='') 
			$temp_poz_sn = $_p_sn;
			
			list($temp_data, $temp_osoba, $temp_umowa, $temp_pion, $temp_up, $temp_rodzaj, $temp_status, $temp_uwagi, $temp_fakt_id, $temp_poz_nazwa)=mysql_fetch_array(mysql_query("SELECT sprzedaz_data, sprzedaz_osoba, sprzedaz_umowa_nazwa, sprzedaz_pion_nazwa, sprzedaz_up_nazwa, sprzedaz_rodzaj, sprzedaz_status, sprzedaz_uwagi, sprzedaz_faktura_id, sprzedaz_pozycja_nazwa FROM $dbname.serwis_sprzedaz WHERE (sprzedaz_pozycja_id='$temp_poz_pid')",$conn));	
						
			//$temp_poz_cena_cr	= $newArray['sprzedaz_pozycja_cenanetto'];
			//$temp_sprzedaz_unique = $newArray['sprzedaz_unique'];
	
			
		if ((($temp_status=='1') || ($temp_status=='9')) && ($temp_data>=$okres_od) && ($temp_data<=$okres_do)) {	
	...
	*/
	...
	errorheader("Podany zakres dat zawiera okres, w którym nie była rejestrowana sprzedaż dla sprzętu po NI i SN");
	?><meta http-equiv="REFRESH" content="<?php echo $http_refresh; ?>;url=<?php echo "$linkdostrony";?>main.php"><?php	
}
startbuttonsarea("right");

echo "<span style='float:left'>";
addlinkbutton("'Zmień kryteria'","main.php?action=zsds&okres_od=$_REQUEST[okres_od]&okres_do=$_REQUEST[okres_do]&sprzet=".urlencode($_REQUEST[sprzet])."");
echo "</span>";
...
br();
pageheader("Pokaż sprzedaż w okresie dla wybranego sprzętu");
starttable("350px");
echo "<form name=ruch action=p_sprzedaz_dla_sprzetu_w_okresie.php method=GET onsubmit=\"if (document.getElementById('sprzet').value=='') { alert('Nie podałeś numeru seryjnego, ani inwentarzowego sprzętu'); document.getElementById('sprzet').focus(); return  false; } return validateForm(); \">";
tbl_empty_row();
	tr_();
		td_colspan(2,'c');
	...
	tr_();
		td_colspan(2,'c');
			echo "<b>Podaj numer seryjny lub inwentarzowy sprzętu<br /><br /></b>";
		_td();
	_tr();
	tr_();
		td_colspan(2,'c');
			echo "<input class=wymagane type=text size=70 maxlength=50 name=sprzet id=sprzet value='$_REQUEST[sprzet]'>";
		_td();
	_tr();
tbl_empty_row();
endtable();
...
?>
<script>
document.getElementById('sprzet').select();
</script>
<?php
}
?>
...
document.getElementById('sprzet').focus();
...
<script>
// hd_get_ni_list <- do dorobienia
/*	$().ready(function() {
		$("#sel_up").autocomplete("hd_get_up_list.php?filia=<?php echo $es_filia; ?>", {
			width: 360,
			max: 150,
			matchContains: false,
			mustMatch: false,
