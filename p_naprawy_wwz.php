...
<?php
if ($es_m==1) {
	$result = mysql_query("SELECT * FROM $dbname.serwis_naprawa WHERE (naprawa_status=0)", $conn) or die($k_b);
} else {
	$result = mysql_query("SELECT * FROM $dbname.serwis_naprawa WHERE (naprawa_status=0) and ((belongs_to=$es_filia) or ((naprawa_przekazanie_naprawy_do=$es_filia) and (naprawa_przekazanie_zakonczone=0)))", $conn) or die($k_b);
}
if (mysql_num_rows($result)!=0) {
	br();
	pageheader("Przeglądanie uszkodzonego sprzętu - naprawy we własnym zakresie",1);
	...
	starttable();
	if ($es_m==1) {
		th("30;c;LP|;;Nazwa Model<br />Numer seryjny, inwentarzowy|;;Pobrano z|;;Przyjęcie sprzętu<br />Data przyjęcia|;c;Sprzęt zastępczy|;c;Uwagi<br /><sub>Filia</sub>|;c;Opcje",$es_prawa);
	} else { 
		th("30;c;LP|;;Nazwa Model<br />Numer seryjny, inwentarzowy|;;Pobrano z|;;Przyjęcie sprzętu<br />Data przyjęcia|;c;Sprzęt zastępczy|;c;Uwagi|;c;Opcje",$es_prawa);	
	}
	$i = 0;
	...
	//	td(";;".$msn."<br />".$mni."");
		...
		td_(";c");
			//if ($mstatus=='0') echo "<a title=' Sprzęt jest naprawiany we własnym zakresie ' href=main.php?action=nwwz&id=$mid&cs=$es><img class=imgoption src=img/snapraw_w.gif border=0></a>";
		/*	echo "<b>";
		...
		*/
			...
			if ($msz>0) {
				...
				if ($naprawa_przekazana_do_innej_filii==1) { 
					if ($n_przekaz_do!=$es_filia)
						if ($n_zgl_id>0) {
							...
							echo "<a title=' Pobierz sprzęt serwisowy $temp_nazwa $temp_model (SN: $temp_sn) z $mup '><input class=imgoption type=image src=img//wycofaj_z_komorki.gif onclick=\"newWindow_r(700,595,'z_naprawy_wycofaj_sprzet_serwisowy.php?id=$mid&cs=$mstatus&tup=".urlencode($mup)."&szid=$msz&sz=$msz')\"></a>";
						}
				} else {
					if ($n_zgl_id>0) {
						echo "<a title=' Pobierz sprzęt serwisowy $temp_nazwa $temp_model (SN: $temp_sn) z $mup '><input class=imgoption type=image src=img//wycofaj_z_komorki.gif onclick=\"newWindow_r(700,595,'z_naprawy_wycofaj_sprzet_serwisowy.php?id=$mid&cs=$mstatus&tup=".urlencode($mup)."&szid=$msz&sz=$msz&hd_zgl_nr=$n_zgl_id&from=hd&up=".urlencode($pelnanazwakomorki)."')\"></a>";
				...
				if ($mnptn!="0000-00-00 00:00:00") {
					...
					if ($cal_dni[0]=='-') {
						echo "<br /><font color=green><b><sub><a title=' Termin naprawy upływa za ".br2nl(substr($cal_dni,1,strlen($cal_dni)))." '>".substr($cal_dni,1,strlen($cal_dni))."</a></sub></b></font>";
					} else {
						echo "<br /><font color=red><b><sub><a title=' Minęło ".br2nl(calculate_datediff($mnptn,$dddd,"d"))." od przewidywanego czasu wykonania naprawy '>Po terminie ".calculate_datediff($mnptn,$dddd,"d")."</a></sub></b></font>";
					}
				}
		...
		//	if ($mstatus==7) echo "<a title=' Zwrot wycofanego sprzętu z serwisu: $mnazwa $mmodel (SN: $msn) '><input class=imgoption type=image src='img//return.gif' onclick=\"newWindow_r(700,595,'z_naprawy_wycofaj.php?id=$mid&status=$mstatus&szid=$msz&tup=".urlencode($mup)."'); return false;\"></a>";
				...
				if ($naprawa_przekazana_do_innej_filii==1) {
					// informacje o tym że sprzęt jest w innej filii / oddziale
						if ($n_przekaz_do==$es_filia) {
							...
							echo "<a title=' Przesuń naprawiany sprzęt do serwisu zewnętrznego '><input class=imgoption type=image src=img//naprawa_przekaz_do_sz.gif onclick=\"newWindow_r(800,600,'z_naprawy_status.php?id=$mid&cs=0&tup=".urlencode($mup)."&from=hd&hdzglid=$n_zgl_id&obcezgloszenie=1&s_docelowy=3A')\"></a>";	
					...
					echo "<a title=' Przesuń naprawiany sprzęt do serwisu zewnętrznego '><input class=imgoption type=image src=img//naprawa_przekaz_do_sz.gif onclick=\"newWindow_r(800,600,'z_naprawy_status.php?id=$mid&cs=0&tup=".urlencode($mup)."&from=hd&s_docelowy=3A&hdzglid=$n_zgl_id')\"></a>";	
	...
	br();
	errorheader("Brak uszkodzonego sprzętu na stanie");
