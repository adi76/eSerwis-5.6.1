...
if ($submit) { 
	...
	if ($_POST[duwagi]!='') {
		...
		$sql_e1="UPDATE $dbname.serwis_naprawa SET naprawa_uwagi_sa = '0' , naprawa_uwagi = '' WHERE naprawa_id = '$uid' LIMIT 1";
	}
...
$result = mysql_query("SELECT naprawa_id,naprawa_nazwa,naprawa_model,naprawa_sn,naprawa_uwagi, naprawa_ni FROM $dbname.serwis_naprawa WHERE (naprawa_id=$id) LIMIT 1", $conn) or die($k_b);
list($temp_id,$temp_nazwa,$temp_model,$temp_sn,$temp_uwagi,$temp_ni)=mysql_fetch_array($result);
pageheader("Edycja uwag o sprzęcie uszkodzonym");
