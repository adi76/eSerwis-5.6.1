...
document.getElementById('sl_tresc').value=opener.document.getElementById('hd_tresc').value;document.getElementById('sl_kat_id').value=opener.document.getElementById('kat_id').value; MakePodkategoriaList_sl(opener.document.getElementById('kat_id').options[opener.document.getElementById('kat_id').options.selectedIndex].value,opener.document.getElementById('podkat_id').value); document.getElementById('sl_podkat_id').value=opener.document.getElementById('podkat_id').value; GenerateSubPodkategoriaList(opener.document.getElementById('kat_id').value,opener.document.getElementById('podkat_id').value); document.getElementById('sl_sub_podkat_id').value=opener.document.getElementById('sub_podkat_id').value;">
<?php 
if ($submit) { 
...
starttable();
echo "<form name=add action=$PHP_SELF method=POST onSubmit=\"return pytanie_zatwierdz_d_slownik('Czy napewno chcesz dodać wybraną treść do słownika ?');\" />";
tbl_empty_row();
tr_();
	td("100;rt;Treść");
	td_(";;");
...
_tr();
	tr_();
		td("150;rt;Kategoria");
		td_(";;;");	
			echo "<select class=wymagane id=sl_kat_id name=sl_kat_id onChange=\"MakePodkategoriaList_sl(this.options[this.options.selectedIndex].value); GenerateSubPodkategoriaList(document.getElementById('sl_kat_id').value,document.getElementById('sl_podkat_id').value);\" onKeyUp=\"if (event.keyCode==13) document.getElementById('sl_podkat_id').focus(); \" />\n";
