...
if (mysql_num_rows($result)!=0) {
	br();
	pageheader("Przeglądanie uszkodzonego sprzętu - pobrany od klienta",1);
...
//			if ($n_przekaz_do==$es_filia) { $naprawa_przekazana_do_innej_filii = 1; } else { $naprawa_przekazana_do_innej_filii = 0; }
		...
		$i++;
		td(";c;<a class=normalfont href=# title=' $mid '>".$change_color_start."".$i."".$change_color_stop."</a>");
				...
				if ($naprawa_przekazana_do_innej_filii==1) {
					// informacje o tym że sprzęt jest w innej filii / oddziale
						if ($n_przekaz_do==$es_filia) {
							...
							echo "<a title=' Naprawiaj '><input class=imgoption type=image src=img//naprawiaj.gif onclick=\"newWindow_r(800,600,'z_naprawy_status.php?id=$mid&cs=$mstatus&tup=".urlencode($mup)."&hdzglid=$n_zgl_id&obcezgloszenie=1')\"></a>";	
					...
					echo "<a title=' Naprawiaj '><input class=imgoption type=image src=img//naprawiaj.gif onclick=\"newWindow_r(800,600,'z_naprawy_status.php?id=$mid&cs=$mstatus&tup=".urlencode($mup)."&hdzglid=$n_zgl_id')\"></a>";				
	...
	errorheader("Brak uszkodzonego sprzętu o wybranym typie na stanie (".$_REQUEST[wybierz_typ].")");
/*
	?>
