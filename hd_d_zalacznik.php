...
if ($_GET[edit]==1) {
	pageheader('Dodawanie załączników do kroku <b>'.$_GET[krok].'</b> zgłoszenia nr <b>'.$_GET[zgl].'</b>',0,0);
} else {
	pageheader('Przeglądanie załączników do kroku <b>'.$_GET[krok].'</b> zgłoszenia nr <b>'.$_GET[zgl].'</b>',0,0);
}
?>
<?php if ($_GET[edit]==1) { ?>
<input name="nameprefix" id="nameprefix" type="hidden" maxlength="255" size="50" value="<?php echo $_GET[zglszcz_id]; ?>" />
...
&nbsp;
<div id="fileUploadgrowl"><h2 style="font-weight:normal">Aby móc załączać pliki, wymagany jest plugin do FlashPlayera.<br /><br />Ściągnij wymagany plugin:
<a class="buttons normalfont" target="_blank" href="download/IE/install_flash_player_ax.exe">Dla Internet Explorer'a</a>
&nbsp;<a class="buttons normalfont" target="_blank" href="download/nonIE/install_flash_player.exe">Ściągnij plugin dla pozostałych przeglądarek</a></h2>
</div>
...
okheader('Lista już dołączonych plików do kroku <b>'.$_GET[krok].'</b> zgłoszenia nr <b>'.$_GET[zgl].'<b/> <input type=button class=buttons value=\'Odśwież listę\' onClick=\'self.location.reload(true); return false;\' />',0,0);
if (strlen($thelist)>161) {
	echo $thelist;
} else echo "<h2 style='font-weight:normal;'>Do tego kroku brak jest załączników</h2>";
