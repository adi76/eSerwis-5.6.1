<?php
$pw = array($_f."/hd_d_zgloszenie.php",$_f."/hd_e_zgloszenie_new.php",$_f."/d_zadanie.php",$_f."/e_zadanie.php",$_f."/hd_d_zgloszenie_simple.php");
if (array_search($PHP_SELF, $pw)>-1) { ?>
	...
	function MakePodkategoriaList_EZ(o) {
...
//			ps.options[ps.options.length] = new Option("Kopie bezpieczeństwa","H",false,false);
	...
	function MakePodkategoriaList(o) {
		...
		if (document.getElementById('status_id').value!='1') {	
			$('#WieleOsob').show();
			$('#WieleOsobWybor').hide();
			document.getElementById('WieleOsobCheck').checked=false;
			//$('#WieleOsobWybor').hide();
			$("#userlist").toChecklist('clearAll');
		} else {
			$('#WieleOsob').hide();
			$('#WieleOsobWybor').hide();
		}
	...
	function MakePodkategoriaList_simple(o) {
		if (document.getElementById('kat_id')) {
			var kateg_select = document.getElementById('kat_id');
		} else {
			var kateg_select = document.getElementById('dz_kat');
	...
	*/	
	...
	function StatusChanged(v, v1, v2) {
		...
		if (v==3) {
			...
			if (v1==1) {
				document.getElementById('RodzajPojazdu').style.display='none';
