function passwordStrength(password){var desc=new Array();desc[0]="Bardzo słaba";desc[1]="Słaba";desc[2]="Całkiem dobra";desc[3]="Średnia";desc[4]="Wysoka";desc[5]="Bardzo wysoka";var score=0;if(password.length>6)score++;if((password.match(/[a-z]/))&&(password.match(/[A-Z]/)))score++;if(password.match(/\d+/))score++;if(password.match(/.[!,@,#,$,%,^,&,*,?,_,~,-,(,)]/))score++;if(password.length>12)score++;document.getElementById("passwordStrength").className="strength"+score;document.getElementById("passwordStrength").title="Siła hasła: "+desc[score];return score;}
function GetKey(evt){var charCode=(evt.which)?evt.which:event.keyCode;if(charCode==13){}
return charCode;}
function InfoAboutPasswordStrength(){if(document.getElementById('username').value==''){alert('Nie wpisałeś nazwy użytkownika');document.getElementById('username').focus();return false;}
if(document.getElementById('password').value==''){document.getElementById('password').focus();return false;}
var wynik=passwordStrength(document.getElementById('password').value);if((wynik==0)||(wynik==1))alert('Siła Twojego hasła jest niska. \r\n\r\n Zalecana jest zmiana hasła na bardziej skomplikowane');return true;}
...
function CheckKey(evt){var charCode=(evt.which)?evt.which:event.keyCode;if(charCode==13){}
return true;}