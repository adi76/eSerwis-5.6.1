	...
	this.each(function() {
			...
			.keyup(function(event) {
				...
				$('label',checklistDivId).each(function() {
					if ( !$(this).is(':disabled') ) {
						...
						if (( curItem.indexOf(typedText) > 0 ) || (( curItem.indexOf(typedText) == 0 ))) { // If the label text begins
		...
		var check = function(event) {
			...
			if (event.type == 'keydown') {
				...
				if ((event.keyCode == 13) && ($('#hd_oz').length > 0)) { $('#hd_oz').focus(); return false; }
				if ((event.keyCode == 13) && ($('#userlist_selectedItems').length > 0)) { $('#userlist_selectedItems').focus(); return false; }
		...
		var showSelectedItems = function() {
			...
			$('label',checklistDivId).each(function() {
				if ($(this).parent().hasClass(o.cssChecked)) {
					...
					$(selectedItemsListId).append(''+labelText+', ');
				}
			});
		};
